...
sub render_ui {
    ...
    # URL absoluta do próprio script — usada pelo JS para as chamadas AJAX.
    my $self_url = $ENV{'SCRIPT_NAME'}
        // '/cgi/addons/usertools/addon_usertools.cgi';
    $self_url =~ s/[<>'"&]//g;

    # Content-type PRECISA ser a primeira saída (o cpsrvd não o injeta).
    # O WHM carrega este CGI em iframe dentro do chrome nativo, desde que
    # o AppConfig não tenha "target=_self" (o default integra via iframe).
    print "Content-type: text/html; charset=utf-8\r\n\r\n";
    print _html_page( $role, $self_url );
}
