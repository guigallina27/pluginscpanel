...
# sprite_generator consolida nosso PNG no icon_spritemap do Jupiter.
# Sem isso o icone nao renderiza - o Jupiter usa um sprite CSS e ignora
# arquivos individuais em application_icons/.
if [[ -x /usr/local/cpanel/bin/sprite_generator ]]; then
    /usr/local/cpanel/bin/sprite_generator >/dev/null 2>&1 || true
    echo "  - Spritemap de icones regenerado (incluindo ${PLUGIN_NAME}.png)"
fi
